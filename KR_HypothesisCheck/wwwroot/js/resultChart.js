...
function conclusionDescription(conclusion) {
    if (conclusion == "Распределение подчиняется нормальному закону") {
        return "Исходя из анализа данных, отклонения производства в пределах нормы. Проведение проверки производственной линии не требуется."
    }

    if (conclusion == "Распределение не подчиняется нормальному закону") {
        return "Исходя из анализа данных выявлены нестандартные статистические значения. Требуется проведение проверки производственной линии."
    }

    if (conclusion == "Распределение подчиняется закону Пуассона") {
        return "Исходя из анализа данных представлены вероятности выпуска бракованного процессора в партии произведенного товара."
    }
...
function DrawTable(json) {
    ...
    if (obj.Conclusion == "Распределение подчиняется нормальному закону" || obj.Conclusion == "Распределение не подчиняется нормальному закону") {
        ...
        addRow(mapArr(obj.StatisticData), obj.LabelData.length, "Эмпирическая вероятность обнаружения определенного количества брака в партии");
